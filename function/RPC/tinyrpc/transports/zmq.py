...
class ZmqServerTransport(ServerTransport):
    ...
    def create(cls, zmq_context, endpoint):
        ...
        print("Create zmq ROUTER at: " + endpoint)
...
class ZmqClientTransport(ClientTransport):
    ...
    def __init__(self, socket, context, endpoint):
        self.socket = socket
        self.context = context
        self.endpoint = endpoint

    def send_message(self, message, expect_reply=True, timeout=1000):
        poll = zmq.Poller()
    ...
    @classmethod
    def create(cls, zmq_context, endpoint,identity):
        ...
        print("Create zmq client REQ(%s) at: %s" %(identity,endpoint))
